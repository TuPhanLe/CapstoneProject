...
uint8_t readDHT11(dht11_t *dht)
{
	...
	uint16_t humValIntegral = 0, tempValIntegral = 0, parityVal = 0, genParity = 0;
	uint16_t humValDec = 0, tempValDec = 0,humVal =0, tempVal =0 ;
	uint16_t mData[40];
...
//	HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET);
	...
	for(int i = 0; i < 8; i++)
	{
		humValIntegral += mData[i];
		humValIntegral = humValIntegral << 1;
	}
	for(int i = 8; i < 16; i++)
	{
		humValDec += mData[i];
		humValDec = humValDec << 1;
	}

	//get temp value from data buffer
	for(int i = 16; i < 24; i++)
	{
		tempValIntegral += mData[i];
		tempValIntegral = tempValIntegral << 1;
	}
	for(int i = 24; i < 32; i++)
	{
		tempValDec += mData[i];
		tempValDec = tempValDec << 1;
	}
	humVal = humValIntegral*100 + humValDec;
	tempVal = tempValIntegral*100 + tempValDec;
